...
export default function AttemptQuiz() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let response;
      // Check if this is an instructor testing the quiz
      if (user?.role === 'instructor') {
        response = await api.mockQuizAttempt(id, attemptData);
      } else {
        response = await api.submitQuizAttempt(id, attemptData);
      }
      ...
      navigate(`/quiz/${id}/results`, { 
        state: { 
          ...
          quiz: quiz,
          isMockAttempt: response.data.isMockAttempt || false
        }
      });
    } catch (error) {
            ...
            <div>
              ...
              {user?.role === 'instructor' && (
                <p className="text-sm text-blue-600 mt-1">
                  🧪 Test Mode - This attempt will not be saved
                </p>
              )}
            </div>
